...
mySushiColors<-function (palette = "fire")
{
    if (palette == 7) { return(colorRampPalette(c("black", "blue", "#5900E5", "#E5001B", "orange", "yellow", "white")))}
    if (palette == 6) { return(colorRampPalette(c("black", "blue", "#5900E5", "#E5001B", "orange", "yellow")))}
    if (palette == 5) { return(colorRampPalette(c("black", "blue", "#5900E5", "#E5001B", "orange")))}
    if (palette == 4) { return(colorRampPalette(c("blue", "#5900E5", "#E5001B", "orange")))}
    if (palette == 3) { return(colorRampPalette(c("blue", "#E5001B", "orange")))}
    if (palette == 2) { return(colorRampPalette(c("blue", "#E5001B")))}
    if (palette == "list") { print((2:7)) }
}
...
function(geneinfo=NULL, chrom=NULL, chromstart=NULL,chromend=NULL,
                      col=mySushiColors(2)(2)[1],bheight=0.3,lheight=0.3,bentline=TRUE,
                      ...
                      labeltext=TRUE,labeloffset=0.4,fontsize=.7,fonttype=2,labelat="middle",...)
{
  ...
  # if the gene info is nullusing current hu  man annotations
  ...
  plottranscript <- function(exons,col,yvalue,bheight,lheight,bentline=TRUE,border="black",
                             arrowlength,bprange,strandlength=0.02,strandarrowlength=0.03,plotgenetype="box",
                             labeltext=TRUE,labeloffset=0.4,fontsize=.7,fonttype=2,labelat="middle",...)
  {
    strand = exons[1,6]
